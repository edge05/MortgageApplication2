		...
		stage("Build") {
            steps {
            	...
            	sh "export DBB_CONF=${env.DBB_CONF}"
 				sh "${env.groovyzHome}/groovyz --classpath .:${env.polyClassPath} $WORKSPACE/build/build.groovy --collection MortgageApplication --sourceDir $WORKSPACE/conf/package2.txt"           
            }
        }
        stage("Test") {
            options {
